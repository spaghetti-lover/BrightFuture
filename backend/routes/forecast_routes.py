...
def classify_weather_forecast_type(df):
    ...
    for interval in config['time_intervals']:
        ...
        try:
            rf_path = os.path.join(classified_weather_dir, f'RF_Weather_{interval}_.pkl')
            grid = joblib.load(rf_path)
            classified_weather_type = grid.predict(interval_dataset[config['predictors']])
        except:
...
def standardize_data_weather_forecast(df):
    ...
    new_stand_df = pd.DataFrame(X_new_test, columns=config['standardize_predictor_list'], index=df.index)
    new_stand_df = pd.concat([new_stand_df, df[['date', 'season', 'weather_type', 'time_interval']]], axis=1, join="inner")
    return new_stand_df
...
def predict_forecast(new_stand_test):
    ...
    for interval, weather_type in product(config['time_intervals'], config['weather_types']):
        ...
        if not X_test.empty:
            model_path = os.path.join(fitted_models_dir, f'XGB_fitted_{interval}_{weather_type}.pkl')
...
async def forecast_energy(request: ForecastRequest):
    ...
    forecast_result = [
        ForecastResponse(date=row["date"].strftime("%Y-%m-%d %H:%M:%S"), PredictedTotalPower=row["PredictedTotalPower"])
